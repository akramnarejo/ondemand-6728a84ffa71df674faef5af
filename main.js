...
async function submitQueryWithSSE(sessionId, query) {
    try {
...
(async () => {
    try {
        ...
        // Submit query with default response mode (sync)
        await submitQuery(sessionId, query);

        // Submit query with SSE (stream mode)
        await submitQueryWithSSE(sessionId, query);
    } catch (error) {
